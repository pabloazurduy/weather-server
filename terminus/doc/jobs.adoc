...
* *Palette*: Synchronizes {trmnl_link} palettes. New palettes, as they become available, are added to the database while existing records are updated instead. By default, this checks for updates once a day.
* *Model*: Synchronizes {trmnl_link} models. New models, as they become available, are added to the database while existing records are updated instead. Each model will have one to many palette names as provided by the Palette synchronizer mentioned above. By default, this checks for updates once a day.
